...
const batchGetSubscribedToUser = async (userIds) => {
  const usersFromCache = (await usersDataLoader.load('ALL_USERS')) as any;
...
export const profileLoader = new DataLoader((ids) => batchGetUserProfile(ids));
export const postLoader = new DataLoader((ids) => batchGetUserPosts(ids));
export const memberTypeLoader = new DataLoader((ids) => batchGetProfilesMemberType(ids));
export const userSubscribedToLoader = new DataLoader((ids) =>
